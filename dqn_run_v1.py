...
def get_state(tasks_list, machines):
    ...
    for i, task in enumerate(tasks_list):
        ...
        task_state.append(task.cpu_utilization_ratio)
        task_state += machines_state
...
if __name__ == '__main__':
    ...
    all_batch_tasks = FileIo("data/create/create_tasks_8.txt").readAllBatchLines()
    print("环境创建成功！")
    ...
    DRL = DQN(cloudletDim, vmsNum, vmDim)
    print("网络初始化成功！")
    ...
    for step, batch_tasks in enumerate(all_batch_tasks):
        ...
        state = get_state(tasks_list, cluster.machines)
        state_all += state
        machines_id = DRL.choose_action(np.array(state))  # 通过调度算法得到分配 id
        ...
        for i, task in enumerate(cluster.finished_tasks[-len(tasks_list):]):  # 便历新提交的一批任务，记录动作和奖励
            ...
            reward_all.append([task.task_response_time])
    ...
    FileIo("result/create/finished_tasks_dqn_8.txt").twoListToFile(finished_tasks, "w")
    print("Good job!")
