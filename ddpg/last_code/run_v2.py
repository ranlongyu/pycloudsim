...
if __name__ == "__main__":
    ...
    for step in range(100000):
        ...
        if (step > 400):
            ...
            # 获取完成了的行，截取最后1000条记录
            new_state = np.array(state_all, dtype=np.float32)[readyLine][-2000:]
            new_action = np.array(action_all, dtype=np.int64)[readyLine][-2000:]
            new_time_state_reward = time_state_reward_all[readyLine][-2000:]
            new_cpu_reward = np.array(cpu_reward_all, dtype=np.float32)[np.array(readyLine) + 1][-2000:]  # 下一个状态的cpu
            ...
            # 下标加1，得到下一个状态
            new_state_ = np.array(state_all, dtype=np.float32)[np.array(readyLine) + 1][-2000:]
